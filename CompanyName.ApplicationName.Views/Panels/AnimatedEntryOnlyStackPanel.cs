...
namespace CompanyName.ApplicationName.Views.Panels
{
    ...
    public class AnimatedEntryOnlyStackPanel : Panel
    {
        ...
        /// </summary>
        public static DependencyProperty OrientationProperty = DependencyProperty.Register(nameof(Orientation), typeof(Orientation), typeof(AnimatedEntryOnlyStackPanel), new PropertyMetadata(Orientation.Vertical));
        ...
        protected override Size ArrangeOverride(Size finalSize)
        {
            ...
            foreach (UIElement child in Children)
            {
                child.Arrange(Orientation == Orientation.Horizontal ? new Rect(child.DesiredSize) : new Rect(0, 0, finalSize.Width, child.DesiredSize.Height));
                AnimatePosition(child, endPosition, TimeSpan.FromMilliseconds(300));
                if (Orientation == Orientation.Horizontal) endPosition.X += child.DesiredSize.Width;
                else endPosition.Y += child.DesiredSize.Height;
            }
            return finalSize;
        }
